...
import { Todo, User } from "../../../hooks/useApiCall";
...
export default function BasicTableOne({ data, user }: { data: Todo[]; user: User }) {
  return (
          ...
          <TableBody className='divide-y divide-gray-100 dark:divide-white/[0.05]'>
            {data?.map((todo) => (
              <TableRow key={todo.id}>
                ...
                <TableCell className='px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400'>
                  <div className='flex -space-x-2'>{todo.createdAt}</div>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
